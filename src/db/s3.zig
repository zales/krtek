...
pub const Rows = struct {
	...
	/// The bucket these rows came from, so the grid knows they can be edited.
	table: []const u8 = "",
	at: usize = 0,
	started: bool = false,
...
test "a listing is read the way S3 sends one" {
	...
	try testing.expectEqualStrings("2015/august trip.jpg", listing.entries[0].key);	try testing.expectEqual(@as(i64, 434234), listing.entries[0].size);
